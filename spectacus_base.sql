...
DROP IF EXISTS stock;
DROP IF EXISTS titre;
DROP IF EXISTS emprunt;
DROP IF EXISTS format;
DROP IF EXISTS client;
DROP IF EXISTS participant;
DROP IF EXISTS individu;
DROP IF EXISTS theme;
...
CREATE TABLE IF NOT EXISTS `titre` (
  ...
  `theme` int COLLATE utf8_unicode_ci NOT NULL,
  PRIMARY KEY (`idTitre`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
ALTER TABLE `titre`
  ADD CONSTRAINT `titre_ibfk_1` FOREIGN KEY(`theme`) REFERENCES `theme` (`idTheme`);
